...
files:
 ...
 - history/prompts/general/1-create-rag-retrieval-validation-spec.general.prompt.md
tests:
 - null
---
...
## OBJECTIVE
Develop a testing script to validate the RAG pipeline. The script must take a user query, generate an embedding using Cohere (search mode), query Qdrant, and output the most relevant text chunks.

## TECH STACK
- **Language:** Python.
- **Embedding:** Cohere (`input_type="search_query"`).
- **Database:** Qdrant Cloud.

## FUNCTIONAL REQUIREMENTS
1.  **Search Logic:** Implement a `retrieve(query)` function.
2.  **Cohere Config:** Ensure the query embedding uses the specific `search_query` input type (critical for v3 models).
3.  **Output:** Print the "Distance Score" and "Payload Content" for the top 3 results to verify relevance.
...
- ✅ Impact: Created comprehensive specification for RAG retrieval validation script with user stories, requirements, and success criteria
- 🧪 Tests: null
- 📁 Files: specs/1-rag-retrieval-validation/spec.md, specs/1-rag-retrieval-validation/checklists/requirements.md
